...
Defines custom exception classes and global exception handlers for SmartScan backend application exception handling.
Ensures consistent error response format.

Provided exception classes:
- CustomException: Base custom exception class
- NotFoundException: 404 resource not found exception
- BadRequestException: 400 bad request exception
- ConflictException: 409 conflict exception
- UnauthorizedException: 401 authentication failed exception

All exceptions are converted to a unified JSON response format.
"""
