...
PYTHONPATH 설정 예시:
    PYTHONPATH=lambdas/chatbot-skill-server pytest lambdas/chatbot-skill-server/tests/
"""
import json
import unittest
...
def _kakao_text(resp: dict) -> str:
    """make_res(kakao=True) 응답에서 텍스트 추출.
    응답 구조: {'statusCode': 200, 'body': '{"version":"2.0","template":{...}}'}
    """
...
class TestHandleChatbot(unittest.TestCase):
    ...
    # 2. get_user_by_kakao_id → None (연결된 기기 없음)
    @patch("services.chatbot_service.get_user_by_kakao_id", return_value=None)
    def test_no_device_linked(self, _mock):
        from services.chatbot_service import handle_chatbot
    ...
    # 7. '지갑 삭제' → deactivate_item 호출 (count=1)
    ...
    # 8. deactivate_item 0 반환 → 찾을 수 없음 메시지
    ...
    # 9. '기기 해제' → delete_all_items + delete_user_device 모두 호출
